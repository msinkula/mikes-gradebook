...
/* The original Author of this plugin is Adam R. Brown http://adambrown.info/b/widgets/category/kb-gradebook/ */
...
function kbgv_plugin_init(){
	...
	// Start The Admin Section of Steps
	function kbgv_admin_page(){		// for teachers to import their gradesheet from excel or whatever
		$postpath = get_settings('siteurl').'/wp-admin/admin.php?page=gradebook';
	...
	function kbgv_selectCoursePage($args){
		...
		// display start page
		$out .= '
			<h2>Step One: Choose or Create Course</h2>
            <br>
	...
	function kbgv_uploadCSVPage($args){
		...
		$out .= '
			...
			<form enctype="multipart/form-data" action="'. $postpath .'&amp;step=3" method="post">
				...
				<p><input name="email" type="text" id="email" value="Student Email" style="width:300px;" /><label for="email"> What is the name of the column that contains emails?</label></p>
	...
	function kbgv_writeMessagePage($args){
		...
		if (is_array($defaultMsg) && array_key_exists('msg',$defaultMsg))
			...
			$defaultMsg = "Dear student,\n\nAs of [DATE], these are the grades I have recorded for you:\n\n[AUTOTABLE]\n\nAssignments are graded on a 4.0 scale and will be marked down 25% for every week they are late.";
	...
	function kbgv_showGradesForm( $postpath ){
		...
		if (!is_array($courses))
			return '<p>You cannot check your grade; your instructor has not uploaded any grade information yet.</p>';
		$out .= '
			<h4>1. Read these first:</h4>
				<ul>
				<li>Passwords are randomly generated by this site.</li>
				<li>Use the <a href="' .$postpath .'kbgv=reqpassword"><blink>password request form</blink></a> to have your password emailed to you.</li>
			<li>The email address used is the email address that you have supplied to the college when you registered for classes.</li>
			<li>To change your email address, <a href="http://www.sccc.premiumdw.com/contact/">please send me an email</a>.</li>
			</ul>
			<h4>2. Then use this:</h4>
			<form method="post" action="' .$postpath .'kbgv=display&amp;kbgv_t='. time() .'">
			<fieldset>
			<table id="form">
			<tr><td><b>Email address:</b> </td><td><input type="text" name="kbemail" id="kbemail" /></td></tr>
			<tr><td><b>Password:</b> </td><td><input type="password" name="kbpwd" id="kbpwd" /></td></tr>
			<tr><td><b>Course:</b> </td><td><select name="kbcourse" id="kbcourse">
		';
		foreach( $courses as $course ){
			$out .= "<option>{$course}</option>\n";
		}
